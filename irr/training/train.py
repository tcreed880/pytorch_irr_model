...
def pick_accel_and_precision() -> Tuple[str, str]:
    ...
    if torch.cuda.is_available():
        # CUDA AMP is solid
        return "gpu", "16-mixed"
    # Apple Silicon (MPS): AMP not stable; use fp32
...
def run_train(cfg: TrainConfig, datamodule: IrrDataModule | None = None) -> Dict[str, Any]:
    ...
    if pos == 0:
        ...
        ratio = neg / max(pos, 1)
        pos_weight_tensor = torch.tensor([ratio], dtype=torch.float32)  # BCEWithLogitsLoss expects [1]
        pi = pos / (pos + neg)
    ...
    # Try to record hyperparams in TB (best-effort)
    try:
    ...
    trainer = pl.Trainer(
        ...
        check_val_every_n_epoch=1,   # patience counts validations
        deterministic=True,
